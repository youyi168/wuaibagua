...
class DivinationEngine:
    ...
    # 金钱卦投掷结果（依据《图解周易》传统规则）
    # 铜钱两面：字面(有字)=阴，背面(无字)=阳
    # 代码中：0=背(阳)，1=字(阴)
    # 三背(000,sum=0)→老阳动，三字(111,sum=3)→老阴动
    # 两背一字(sum=1,阳多)→少阳，一背两字(sum=2,阴多)→少阴
    COIN_RESULTS = {
        0: ('老阳', '━━━○', '阳动', 9),  # 三背(0+0+0=0)→老阳（动爻）
        1: ('少阳', '━━━', '阳', 7),     # 两背一字(0+0+1=1)→少阳
        2: ('少阴', '━ ━', '阴', 8),     # 一背两字(0+1+1=2)→少阴
        3: ('老阴', '━ ━×', '阴动', 6)   # 三字(1+1+1=3)→老阴（动爻）
    }
    ...
    def _get_trigram(self, yao1, yao2, yao3):
        """根据三爻确定八卦（依据先天八卦顺序）
        参数：yao1=初爻(下), yao2=二爻(中), yao3=三爻(上)
        返回：八卦索引（0=乾, 1=兑, 2=离, 3=震, 4=巽, 5=坎, 6=艮, 7=坤）
        """
        # 阳=1, 阴=0
        bit1 = 1 if '阳' in yao1['yin_yang'] else 0  # 初爻(下)
        bit2 = 1 if '阳' in yao2['yin_yang'] else 0  # 二爻(中)
        bit3 = 1 if '阳' in yao3['yin_yang'] else 0  # 三爻(上)
        
        # 二进制（从上往下）：上爻*4 + 中爻*2 + 初爻*1
        index = bit3 * 4 + bit2 * 2 + bit1
        
        # 先天八卦映射（依据《图解周易》）
        # 乾☰(阳阳阳)=7→0, 兑☱(阴阳阳)=3→1, 离☲(阳阴阳)=5→2, 震☳(阴阴阳)=1→3
        # 巽☴(阳阳阴)=6→4, 坎☵(阴阳阴)=2→5, 艮☶(阳阴阴)=4→6, 坤☷(阴阴阴)=0→7
        trigram_map = {7: 0, 3: 1, 5: 2, 1: 3, 6: 4, 2: 5, 4: 6, 0: 7}
