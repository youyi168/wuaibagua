...
# -*- coding: utf-8 -*-
"""
我爱八卦 - 金钱卦算卦软件 (Kivy 版本 - 响应式 UI 优化版)
适用于打包成安卓 APK
支持多屏幕分辨率自适应
"""
...
class GuaDisplay(GridLayout):
    ...
    def __init__(self, title="", responsive=None, **kwargs):
        ...
        self.cols = 1
        self.spacing = self.responsive.get_spacing(2)
        ...
        self.gua_name_label = Label(
            text="未起卦",
            font_size=self.responsive.get_font_size(22),
            size_hint_y=None,
...
class MainScreen(BoxLayout):
    ...
    def __init__(self, responsive=None, **kwargs):
        ...
        self.result_label = Label(
            ...
            size_hint_y=None,
            padding=(self.responsive.get_padding(10), self.responsive.get_padding(10)),
            markup=True,
            valign='top',
            halign='left'
        )
...
class WuaibaguaApp(App):
    ...
    def on_window_resize(self, window, width, height):
        ...
        # 延迟触发 UI 更新（避免频繁重绘）
        Clock.schedule_once(lambda dt: self.update_ui_scale(), 0.1)
    
    def update_ui_scale(self):
        """更新 UI 缩放（可选：实现完整的动态重绘逻辑）"""
        # 这里可以添加更复杂的 UI 更新逻辑
        # 目前 Kivy 会自动处理大部分响应式布局
        pass
